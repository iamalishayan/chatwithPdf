...
async def get_embedding(text: str) -> List[float]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        raise HTTPException(
            status_code=502, detail="Failed to retrieve embeddings from Gemini API."
        )


async def stream_answer(
    context: str, question: str, sources: List[Dict[str, Any]]
        ...
        try:
            async with client.stream(
                ...
                if response.status_code != 200:
                    ...
                    logger.error(
                        f"Gemini API returned error code {response.status_code}: {error_detail}"
                    )
